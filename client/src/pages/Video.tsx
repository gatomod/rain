...
const Video = ({ uuids }: { uuids: string[] }) => {
    ...
    return (
        <div className='flex gap-y-4 flex-col lg:flex-row justify-around gap-x-8'>
            <div className='flex lg:px-4 lg:pt-6 top-0 z-40 lg:z-0 lg:top-24 flex-col w-full lg:w-3/4 h-max lg:h-min flex-grow gap-y-4 bg-slate-700 lg:bg-gray-100'>
                <div className='flex h-1/3 grow justify-center aspect-video bg-black lg:rounded-lg'>
                    {startVideo ? (
                        <ReactHlsPlayer
                            playerRef={video}
                        ...
                        <img src={data.thumbnail} className='lg:w-max bg-black rounded-lg aspect-video'></img>
                    )}
                    {!startVideo && (
                        <PlayCircle48Filled
