...
/// ## Large thumbnail handler
/// Serve large thumbnail
pub async fn handler(PathURI(uuid): PathURI<String>) -> impl IntoResponse {
    if !Path::new(&format!("../cdn/{}/thumbnail_lg.png", uuid)).exists() {
        return Err(not_found());
    }

    let file = match File::open(format!("../cdn/{}/thumbnail_lg.png", uuid)).await {
        Err(_) => {
